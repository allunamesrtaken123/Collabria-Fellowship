from importAgentsEV import Agent, Texas_Hold_Em, playHand, initializeRothErevWeighted
...
directory = f'August/Investigating Rational/Results/With AAo/Nonlinear'

iterations = 100
startStack = 10000 #10 thousand (doing 1000 because it makes it much faster)
...
for it in tqdm(range(iterations)):
    ...
    for i in range(total_agents):
        agent_pool.append(Agent(ID = f'{i}', stack = startStack, strategy = initializeRothErevWeighted(strategies,startStack)))
    ...
    for e in range(epochs):
        for g in range(int(len(agent_pool)/2)):
            ...
            if whoWon == 0:
                p1.RothErevWeightedLearn()
            else:
                p2.RothErevWeightedLearn()
